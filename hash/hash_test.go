...
func testHash(t *testing.T, test testBlakes) {
	if test.Key == nil {
		...
		hash, err := test.Algo.Hmac(test.Key, test.Data)
		r.NoError(t, err)
		t.Log("Hash: ", hex.EncodeToString(hash))
		r.Equal(t, test.Expected, hash)

		validate, err := test.Algo.ValidateHmac(test.Key, test.Data, hash)
		r.NoError(t, err)
		r.True(t, validate)
	}
}
...
func TestFaultBlakes(t *testing.T) {
	...
	for _, test := range tests {
		testHashFault(t, test)
	}

}

func testHashFault(t *testing.T, test testBlakes) {
	if test.Key == nil {
		validate, err := test.Algo.ValidateHash(test.Data, test.Expected)
		r.NoError(t, err)
		r.False(t, validate)
	} else {
		validate, err := test.Algo.ValidateHmac(test.Key, test.Data, test.Expected)
		r.NoError(t, err)
		r.False(t, validate)
	}
}
